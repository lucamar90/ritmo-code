...
- Refresh **interval** from 30 s to 30 min, auto **slideshow**, **brightness** (also automatic: full by day, low from sunset to sunrise or 20→07 / 21→07), **timezone** (Rome with automatic DST, or GMT±N).
...
- **right-click → Timer e pomodoro** starts a pomodoro (25/5, 50/10, 15/3) or a timer on the device, stops it or skips the phase; the running timer is shown at the top.
...
notification and the sound type: **Windows sounds** (default, they follow the *System sounds* volume and
work over remote desktop too) or **Ritmo Code melodies** (with their own volume); **Prova** plays a sample. On the device it is toggled in *Settings → sounds on
