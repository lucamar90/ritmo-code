...
- **`ritmo_code/`** — il progetto. Sketch arduino-cli completo: recupera l'utilizzo di
  ...
  (AES-256-GCM + PIN). Per la build vedi [`ritmo_code/build.sh`](ritmo_code/build.sh).
- **`bringup/`** — bring-up validato sull'hardware (colori corretti, orientamento
  ...
  (qui non c'è un `partitions.csv`) e riutilizza il `lv_conf.h` di `ritmo_code`;
