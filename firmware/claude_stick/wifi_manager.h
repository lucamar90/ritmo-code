...
public:
    ...
    bool begin() {
        ...
        _loadAll();
        WiFi.mode(WIFI_STA);
        // niente risparmio energetico della radio: con il modem sleep le risposte arrivano in
        // 100-300 ms e le connessioni in ingresso (pannello web) o verso il PC vanno in timeout
        WiFi.setSleep(false);
        WiFi.disconnect();
        return true;
    }

    // Prova ogni rete salvata finche' una non si connette.
    //
    // `tick` viene chiamato durante l'attesa, ogni ~100ms, con la rete in prova. Esiste
    // perche' questa funzione blocca fino a timeout_ms PER RETE — con le 3 salvate di
    // MAX_SAVED_NETWORKS sono 24s — e chi la chiama al boot ha bisogno di qualcuno che faccia
    // girare LVGL in quell'intervallo, altrimenti lo schermo resta bloccato finche' non parte loop().
        ...
        for (int i = 0; i < _count; i++) {
            ...
            if (WiFi.status() == WL_CONNECTED) {
                ...
                Serial.printf("WiFi: connected to '%s'! IP=%s RSSI=%d dBm\n",
                    _nets[i].ssid, WiFi.localIP().toString().c_str(), WiFi.RSSI());
                if (i > 0) _promote(i);
                return true;
            }
            WiFi.disconnect();
    ...
    // rende la rete idx la prima provata all'avvio
    void promote(int idx) { _promote(idx); }
